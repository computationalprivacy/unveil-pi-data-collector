...
class HostAP(WiFi):
    ...
    def __init__(
                self, interface, ssid='MyWiFi', channel=6,
    ...
    def get_hostapd_conf(self):
        ...
        hostapd_conf_rules = [
            ...
            'deny_mac_file={}'.format(self.mac_deny_path),
            'auth_algs=1',
            'wpa=2',
            'wpa_passphrase={}'.format(self.wifi_ap_password),
    ...
    def hostapd_control(self, action):
        """Control hostapd service."""
        return "systemctl {} hostapd.service && sleep 2".format(action)
    ...
    def verify_hostap_password(self, value):
        ...
        return self.re_search(
            "(?<=^wpa_passphrase=).*", self.hostapd_path) == value
    ...
    def get_connected_users_advanced(self):
        ...
        output = self.execute_command('iw dev {} station dump'.format(
            self.interface)).decode('utf8').strip().lower().split('\n')
        connected_mac = []
        mac = None
        for line in output:
